'''Script to train analog fuelgauge for EU VAns'''
...
def get_config(model_name, imgsize):
    ...
    if model_name.endswith("n"):
        model_cfg_file = "yolov8n-pose.yaml"
        lr = 0.001 #0.001
        if imgsize == 224:
            bsize = 256
        elif imgsize == 320:
...
project_name = "analog_fuelgauge" # the dir where results in yolo_runs will be stored.
...
# HYPERPARAMETERS
epochs = 150 #150 
patience = 50 # After how many epochs to stop training if results do not improve,.
train_versions = ["v1_n"]         #["v1_n", "v1_p_n", "v1_s", "v1_p_s"]  #-original
imgsizes = [320]    #-original
